import { createSlice, nanoid } from "@reduxjs/toolkit";

export const initialState = [
  {
    createdAt: "2023-06-17T22:22:29.497Z",
    name: "Pamela Hane",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/661.jpg",
    age: 53,
    address: "1 Maple St, Anytown, USA",
    id: "1",
  },
  {
    createdAt: "2023-06-18T02:13:38.110Z",
    name: "Shaun Kertzmann",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/911.jpg",
    age: 85,
    address: "2 Maple St, Anytown, USA",
    id: "2",
  },
  {
    createdAt: "2023-06-18T03:46:43.649Z",
    name: "Jose D'Amore",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/138.jpg",
    age: 19,
    address: "3 Maple St, Anytown, USA",
    id: "3",
  },
  {
    createdAt: "2023-06-18T01:52:12.232Z",
    name: "Mr. Lucy Braun",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/268.jpg",
    age: 19,
    address: "4 Maple St, Anytown, USA",
    id: "4",
  },
  {
    createdAt: "2023-06-18T04:27:39.163Z",
    name: "Armando Wiegand",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/154.jpg",
    age: 77,
    address: "5 Maple St, Anytown, USA",
    id: "5",
  },
  {
    createdAt: "2023-06-17T06:09:02.875Z",
    name: "Rudolph Emard MD",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/477.jpg",
    age: 4,
    address: "6 Maple St, Anytown, USA",
    id: "6",
  },
  {
    createdAt: "2023-06-18T01:22:38.127Z",
    name: "Krista O'Hara",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/332.jpg",
    age: 83,
    address: "7 Maple St, Anytown, USA",
    id: "7",
  },
  {
    createdAt: "2023-06-17T04:53:52.740Z",
    name: "Kathleen Lesch",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/766.jpg",
    age: 76,
    address: "8 Maple St, Anytown, USA",
    id: "8",
  },
  {
    createdAt: "2023-06-17T08:07:30.884Z",
    name: "Allan Veum",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/832.jpg",
    age: 2,
    address: "9 Maple St, Anytown, USA",
    id: "9",
  },
  {
    createdAt: "2023-06-17T21:31:30.402Z",
    name: "Kristine Kulas",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/576.jpg",
    age: 65,
    address: "10 Maple St, Anytown, USA",
    id: "10",
  },
  {
    createdAt: "2023-06-17T12:29:51.542Z",
    name: "Marvin Frami",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/429.jpg",
    age: 28,
    address: "11 Oak Ave, Anytown, USA",
    id: "11",
  },
  {
    createdAt: "2023-06-17T08:58:01.792Z",
    name: "Dennis Beer",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/1037.jpg",
    age: 28,
    address: "12 Oak Ave, Anytown, USA",
    id: "12",
  },
  {
    createdAt: "2023-06-17T10:32:46.877Z",
    name: "Gabriel McGlynn",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/987.jpg",
    age: 99,
    address: "13 Oak Ave, Anytown, USA",
    id: "13",
  },
  {
    createdAt: "2023-06-18T00:29:23.638Z",
    name: "Gregory Mann",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/1098.jpg",
    age: 45,
    address: "14 Oak Ave, Anytown, USA",
    id: "14",
  },
  {
    createdAt: "2023-06-18T02:41:06.516Z",
    name: "Traci Thompson DVM",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/237.jpg",
    age: 63,
    address: "15 Oak Ave, Anytown, USA",
    id: "15",
  },
  {
    createdAt: "2023-06-17T06:42:26.772Z",
    name: "Billy Johns",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/967.jpg",
    age: 46,
    address: "16 Oak Ave, Anytown, USA",
    id: "16",
  },
  {
    createdAt: "2023-06-17T13:43:33.447Z",
    name: "Nick Padberg",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/554.jpg",
    age: 36,
    address: "17 Oak Ave, Anytown, USA",
    id: "17",
  },
  {
    createdAt: "2023-06-17T21:27:12.228Z",
    name: "Bryan Bartell",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/257.jpg",
    age: 10,
    address: "18 Oak Ave, Anytown, USA",
    id: "18",
  },
  {
    createdAt: "2023-06-17T05:58:49.932Z",
    name: "Frank Bednar",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/709.jpg",
    age: 20,
    address: "19 Elm St, Anytown, USA",
    id: "19",
  },
  {
    createdAt: "2023-06-17T08:55:26.467Z",
    name: "Kent Stanton",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/1051.jpg",
    age: 54,
    address: "20 Elm St, Anytown, USA",
    id: "20",
  },
  {
    createdAt: "2023-06-17T09:24:15.808Z",
    name: "Jean Windler",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/156.jpg",
    age: 14,
    address: "21 Elm St, Anytown, USA",
    id: "21",
  },
  {
    createdAt: "2023-06-17T07:31:44.280Z",
    name: "Kyle Rowe",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/480.jpg",
    age: 8,
    address: "22 Elm St, Anytown, USA",
    id: "22",
  },
  {
    createdAt: "2023-06-17T18:24:09.589Z",
    name: "Hope Jenkins",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/183.jpg",
    age: 98,
    address: "23 Elm St, Anytown, USA",
    id: "23",
  },
  {
    createdAt: "2023-06-17T16:33:23.539Z",
    name: "Luz Stehr",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/323.jpg",
    age: 86,
    address: "24 Elm St, Anytown, USA",
    id: "24",
  },
  {
    createdAt: "2023-06-17T18:19:20.927Z",
    name: "Shawna Hartmann Sr.",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/15.jpg",
    age: 76,
    address: "25 Elm St, Anytown, USA",
    id: "25",
  },
  {
    createdAt: "2023-06-17T16:33:48.801Z",
    name: "Dr. Jody Bruen III",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/551.jpg",
    age: 93,
    address: "26 Elm St, Anytown, USA",
    id: "26",
  },
  {
    createdAt: "2023-06-17T11:09:20.496Z",
    name: "Kristen Muller",
    avatar:
      "https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/1215.jpg",
    age: 7,
    address: "27 Elm St, Anytown, USA",
    id: "27",
  },
];

const usersSlice = createSlice({
  name: "user",
  initialState,
  reducers: {
    addUser: {
      reducer(state, action){
        state.push(action.payload);
      },
      prepare: (name, avatar, age, address) => {
        return {
          payload: {
            id: nanoid(),
            createdAt: new Date().toISOString(),
            name, 
            avatar,
            age,
            address
          },
        };
      },
    },
    deleteUser: (state, action) => {
        state.splice(action.payload,1);
    }
  },
});
export const { addUser, deleteUser } = usersSlice.actions
export default usersSlice.reducer;
